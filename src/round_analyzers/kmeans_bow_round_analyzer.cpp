...
RoundObservation KMeansBowRoundAnalyzer::analyze(
    ...
    // Find the briscola
    std::optional<CardBBox> briscolaBox = briscola::findBBox({video}, 0, 0, debug, cv::Mat());
    ...
    // Difference among card 2 played frame and start frame
    ...
    // CARD 1
    // K-Means will look only on the movement aera wrt the first frame.
    ...
    // CARD 2
    // Resolve bug of overlapped cards. absdiff(frame2, frame1) shows only the delta 
    // (new card). Even if it covers card 1, K-Means will focus only on the new object.
